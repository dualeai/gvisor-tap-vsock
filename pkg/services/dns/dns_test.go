...
func TestDNSOutboundAllowCaseNormalization(t *testing.T) {
	...
	server, err := NewWithUpstreamResolver(nil, nil, nil, upstream, nil, allowlist)
	require.NoError(t, err)
...
func startDNSServer(upstream upstreamResolver, outboundAllow []*regexp.Regexp) (string, func(), error) {
	...
	server, err := NewWithUpstreamResolver(udpConn, tcpLn, nil, upstream, nil, outboundAllow)
	if err != nil {
		return "", nil, err
	}
