...
func addServices(configuration *types.Configuration, s *stack.Stack, ipPool *tap.IPPool) (http.Handler, error) {
	...
	dnsMux, err := dnsServer(configuration, s, dnsCache, outboundAllow)
	if err != nil {
		return nil, err
	}
...
func dnsServer(configuration *types.Configuration, s *stack.Stack, cache *dnscache.DNSCache, outboundAllow []*regexp.Regexp) (http.Handler, error) {
	...
	server, err := dns.New(udpConn, tcpLn, configuration.DNS, cache, outboundAllow)
	if err != nil {
		return nil, err
	}
